...
class AllWeather(Dataset):
    ...
    def __init__(self, dataset_root: str, task: str = 'derain', split: str = 'train',
                 transforms=None, crop_size: int = 256, random_crop_ratio: tuple = None):
        self.dataset_root = dataset_root
        self.lq_list = []  # low-quality input
        self.gt_list = []  # output
        self.task_id_list = []
        self.task = task
        self.raindrop_aug_factor = 10  #
      
        self.task_to_test_dir = {
            'derain': 'outdoor_rain',
            'deraindrop': 'raindrop',
            'desnow': 'Snow100K',  # Snow100K-L
            #'dehaze': 'OTS',
        }


        if random_crop_ratio is not None:
            self.crop_size = int(random_crop_ratio[0])
        else:
            self.crop_size = crop_size  

        name_file = os.path.join(dataset_root, 'allweather.txt')
        if not os.path.isfile(name_file):
            raise FileNotFoundError(f"allweather.txt not found in {dataset_root}")
        with open(name_file, 'r') as f:
            valid_names = set(line.strip() for line in f)

        if split == 'test':
            if task in self.task_to_test_dir:
                split_name = os.path.join('test', self.task_to_test_dir[task])
            else:
                # fallback to original behavior if not mapped
                split_name = os.path.join('test', task.replace('de', ''))
        elif 'real' in split:
            split_name = 'real'
        else:  # train, val
            # split_name = split
        ...
        if split_name == 'train':
            lq_dir_path = os.path.join(dataset_root, split_name, 'input')
            lq_name_list = [name for name in os.listdir(lq_dir_path) if name in valid_names]  # 过滤
        elif 'real' in split_name:
            lq_dir_path = os.path.join(dataset_root, split_name, task.replace('de', ''))
            lq_name_list = os.listdir(lq_dir_path)
        else:
            lq_name_list = os.listdir(lq_dir_path)

        raindrop_pattern = re.compile(r'^\d+_rain\.png$')
        ...
        for idx, lq_name in enumerate(lq_name_list):
            ...
            if split == 'real':
                gt_path = gt_path.replace(task.replace('de', ''), 'gt')
            
            is_raindrop = False
            if raindrop_pattern.match(lq_name):
                
                num = int(lq_name.split('_')[0])
                if 0 <= num <= 860:
                    is_raindrop = True

            task_id = get_task_info(dataset='allweather', type='idx', task=task)

            if split == 'train' and is_raindrop:
                add_times = self.raindrop_aug_factor
            else:
                add_times = 1
    ...
    def __getitem__(self, idx):
        ...
        # ========== 关键修改：调用重构后的random_crop ==========
        if self.split == 'train':  # 仅训练阶段裁剪
            lq, gt = random_crop(lq, gt, crop_size=self.crop_size)
...
class LowLevel_Cityscapes(Dataset):
    ...
    def __init__(self, dataset_root: str, task: str = 'deblur', split: str = 'train',
        ...
        if random_crop_ratio is not None:
            ...
            self.crop_size = crop_size  # 优先使用新的crop_size参数        
...
class CDD11(Dataset):
    ...
    def __init__(self, dataset_root: str, task: str = 'derain', split: str = 'train',
        ...
        if split == 'test':
            split_name = '{}'.format(split)
        else:  # train, val
            split_name = split

        type_dir_path = os.path.join(dataset_root, split_name)
        if split == 'train':
            type_name_list = [item for item in os.listdir(type_dir_path) if item != 'clear']
        else:
            # type_name_list = [item for item in os.listdir(type_dir_path) if item == task]
            type_name_list = [item for item in os.listdir(type_dir_path) if item != 'clear']
        print(type_name_list)
        for _, type in enumerate(type_name_list):
            lq_dir_path = os.path.join(dataset_root, split_name, type)
            lq_name_list = os.listdir(lq_dir_path)

            for idx, lq_name in enumerate(lq_name_list):
                lq_path = os.path.join(lq_dir_path, lq_name)
                gt_path = lq_path.replace(type, 'clear')
                self.lq_list.append(lq_path)
                self.gt_list.append(gt_path)
                task_id = get_task_info(dataset='cdd11', type='idx', task=task)
                self.task_id_list.append(task_id)
